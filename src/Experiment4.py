...
#identify parents and children goals
...
#set the c-semiring type
csp = WCSP()

#check the satisfiability level of each children goal
MAX_VAR_NUM = 1
...
#check the satisfiability of parents goals
...
nx.draw_circular(G, labels=labels, with_labels=True,font_weight='normal',arrowsize=15, font_size=15)
plt.show()

#====algorithm 2====#
#read goal satisfiability conducted by another actor
...
#compute the satisfiability level from the new agent
...
remove = []
for i in range(len(data_new_goal)):
    if csp.additive([data_new_goal.iloc[i,3],data_goal.iloc[data_new_goal.iloc[i,0],3]]) == data_new_goal.iloc[i,3]:
        candidates.append(i)

#select the candidate providing the highest satisfiability goals
for i in range(len(candidates)):
    for j in range(len(candidates)):
        if (i != j) and (csp.additive([data_new_goal.iloc[candidates[i],3], data_new_goal.iloc[candidates[j],3]]) == data_new_goal.iloc[candidates[i],3]) and (data_new_goal.iloc[candidates[i],3] != data_new_goal.iloc[candidates[j],3]):
            remove.append(j)                                                                                                    

candidates_removed = []
for i in range(len(candidates)):
    if candidates[i] not in remove:                                               
        candidates_removed.append(candidates[i])

#among the goals, one of them is assigned to the new agent
#The goal whose satisfiability level is the lowest conducted by the current agent is replaced 
selected = 0
for i in range(len(candidates_removed)):
    if i == 0:
        selected = candidates_removed[i]
    else:
        if data_goal.iloc[data_new_goal.iloc[selected,0],3] > data_goal.iloc[data_new_goal.iloc[candidates_removed[i],0],3]:
            selected = candidates_removed[i]

data_goal.iloc[data_new_goal.iloc[selected,0],:] = data_new_goal.iloc[selected,:]
        
#reset the satisfiability of parents goals for updating
for i in range(len(goal_parents_original)):
    data_goal.iloc[goal_parents_original[i],-1] = np.nan

#calculate the satisfiability of parent goals
goal_parents = goal_parents_original.copy()
while True:
    if len(goal_parents) == 0:
        break
