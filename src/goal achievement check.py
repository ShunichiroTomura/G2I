...
if MODE == 1:
    ...
    for a in range(REPEAT):
        ...
        #constraint check for all the values in each goal
        for i in range(len(target_goals)):
            qcsp.constraint_check(target_goals[i], goals_actual[i])
        ...
        def constraint_check(self, goal, elements, probability):
            ...
            for i in range(len(goal.c_elements)):
               ...
               if elements[i] == 'E':
                   ...
                   #then the probability takes 1-P(constraint)
    ...
    for a in range(REPEAT):
        ...
        #probability for all constraint values
        probability =[]
        for i in range(len(data)-1):
            probability.append([0.7,0.7,0.7,0.7,0.7])
        ...
        for i in range(len(goals_actual)):
            fcsp.constraint_check(target_goals[i], goals_actual[i],fuzzy_levels[i])
        for i in range(len(goals_actual)):
            wcsp.constraint_check(target_goals[i], goals_actual[i],weight_fuzzy_levels[i],weights[i])
        for i in range(len(target_goals)):
            qcsp.constraint_check(target_goals[i], goals_actual_qua[i])
        for i in range(len(goals_actual)):
            pcsp.constraint_check(target_goals[i], goals_actual_prob[i],probability[i])
